...
router.get('/addPolicy',function(req,res){
  let user = req.session.user;
  res.render('admin/addPolicy',{user})


})
router.post('/addPolicy',function(req,res,next){
  var mail=req.session.user.email;
  let user = req.session.user;
  var id =user.id;
  req.body.cid=id;
    let sql = "INSERT INTO addpolicy SET ?";
  let query = con.query(sql,req.body,(err,result)=>{
    if(err){
      console.log(err)
    }else{
      res.render("admin/addpolicy")
    }
  })
...
router.get('/myPolicy',function(req,res,next){
var sql="Select * from addpolicy  where cid = ?"
...
con.query(sql,[id],(err,row)=>{
  if(err){
    ...
    var user = req.session.user;
    res.render("admin/myPolicy",{data:row,user})
  }
})
})
